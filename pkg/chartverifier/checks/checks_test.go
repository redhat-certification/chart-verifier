...
func TestHasReadme(t *testing.T) {
	...
	for _, tc := range positiveTestCases {
		t.Run(tc.description, func(t *testing.T) {
			config := viper.New()
			r, err := HasReadme(&CheckOptions{URI: tc.uri, ViperConfig: config})
			require.NoError(t, err)
			require.NotNil(t, r)
			require.True(t, r.Ok)
...
func TestContainsTest(t *testing.T) {
	...
	for _, tc := range positiveTestCases {
		t.Run(tc.description, func(t *testing.T) {
			config := viper.New()
			r, err := ContainsTest(&CheckOptions{URI: tc.uri, ViperConfig: config})
			require.NoError(t, err)
			require.NotNil(t, r)
			require.True(t, r.Ok)
...
func TestHasValuesSchema(t *testing.T) {
	...
	for _, tc := range positiveTestCases {
		t.Run(tc.description, func(t *testing.T) {
			config := viper.New()
			r, err := ContainsValuesSchema(&CheckOptions{URI: tc.uri, ViperConfig: config})
			require.NoError(t, err)
			require.NotNil(t, r)
			require.True(t, r.Ok)
...
func TestHasValues(t *testing.T) {
	...
	for _, tc := range positiveTestCases {
		t.Run(tc.description, func(t *testing.T) {
			config := viper.New()
			r, err := ContainsValues(&CheckOptions{URI: tc.uri, ViperConfig: config})
			require.NoError(t, err)
			require.NotNil(t, r)
			require.True(t, r.Ok)
...
func TestHasMinKubeVersion(t *testing.T) {
	...
	for _, tc := range positiveTestCases {
		t.Run(tc.description, func(t *testing.T) {
			config := viper.New()
			r, err := HasKubeVersion(&CheckOptions{URI: tc.uri, ViperConfig: config})
			require.NoError(t, err)
			require.NotNil(t, r)
			require.True(t, r.Ok)
...
func TestNotContainCRDs(t *testing.T) {
	...
	for _, tc := range positiveTestCases {
		t.Run(tc.description, func(t *testing.T) {
			config := viper.New()
			r, err := NotContainCRDs(&CheckOptions{URI: tc.uri, ViperConfig: config})
			require.NoError(t, err)
			require.NotNil(t, r)
			require.True(t, r.Ok)
...
func TestNotContainCSIObjects(t *testing.T) {
	...
	for _, tc := range positiveTestCases {
		t.Run(tc.description, func(t *testing.T) {
			config := viper.New()
			r, err := NotContainCSIObjects(&CheckOptions{URI: tc.uri, ViperConfig: config})
			require.NoError(t, err)
			require.NotNil(t, r)
			require.True(t, r.Ok)
...
func TestHelmLint(t *testing.T) {
	...
	for _, tc := range positiveTestCases {
		t.Run(tc.description, func(t *testing.T) {
			config := viper.New()
			r, err := HelmLint(&CheckOptions{URI: tc.uri, ViperConfig: config})
			require.NoError(t, err)
			require.NotNil(t, r)
			require.True(t, r.Ok)
...
func TestImageCertify(t *testing.T) {
	...
	for _, tc := range testCases {
		t.Run(tc.description, func(t *testing.T) {
			config := viper.New()
			r, err := ImagesAreCertified(&CheckOptions{URI: tc.uri, ViperConfig: config})
			require.NoError(t, err)
			require.NotNil(t, r)
			if tc.numErrors == 0 {
				require.True(t, r.Ok)
